...
public class PlayerController : MonoBehaviour {
	...
	public Sprite[] availablePlayerImage; 
	...
	void Awake () {
        ...
        {
            case 1:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[1];
                break;
            case 2:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[2];
                break;
            case 3:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[3];
                break;
            case 4:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[4];
                break;
            case 5:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[5];
                break;
            case 6:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[6];
                break;
            case 7:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[7];
                break;
            default:
                GetComponent<SpriteRenderer>().sprite = availablePlayerImage[0];
                break;
        }
    }
	...
	void moveAroundOrbit() {
		...
		if(dir == 1)
			transform.rotation = Quaternion.Euler(0, 0, rotationBaseTime/2 * (rotationSpeed*-1 + (float)GameController.level/20) * Mathf.PI*30);
		else
			transform.rotation = Quaternion.Euler (0, 180, rotationBaseTime/2 * (rotationSpeed *-1+ (float)GameController.level/20) * Mathf.PI*30);
	...
	void OnCollisionEnter(Collision c) {
        ...
        if (c.collider.gameObject.tag == "Ghalb")
        {
            GameController.Ghalb++;
            GameController.remainingCoins--;
            PlayerPrefs.SetInt("Ghalb", GameController.Ghalb);
            print(PlayerPrefs.GetInt("Ghalb"));
            Destroy(c.collider.gameObject);
        }
	}
